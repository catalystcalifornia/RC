...
This is a personal package we use for Race Counts indicator prep. It contains some helpful functions that calculates the number of rate values an indicator has, the index of disparity, and more. 

For example, let's suppose we are working with the Life Expectacy indicator from the Health Care Access Issue Area. This is an indicator where we want the highest Life Expectancy for each unit of analysis(county, state, or city). The 'best' here would be the maximum. Other indicators like School Suspension would require us to set the best as 'Minimum' because we would classify the lowest number of suspensions as the best performance.
...
d <- data.frame (geoid = c("06037", "06001", "06113", "06101", "06091"),
                    ...
                    asian_rate = c("43.70", "80.03", "2.99", "2.93", "32.55"))

```

county_id     | county_name   | total_rate    | aian_rate    | black_rate   | latino_rate   | nh_white_rate | asian_rate
------------- | ------------- | ------------- |------------- |------------- | ------------- | ------------- |-------------
06037         | Los Angeles   | 35.70         | 44.9         | 65.70        | 77.70         | 23.70         | 43.70
...
Now let's calculate the commonly used Race-Count analysis. For example, let's calculate the number of rate values per county.  
...
------------- | ------------- | ------------- |------------- |------------- | ------------- | ------------- |-------------| -------------
06037         | Los Angeles   | 35.70         | 44.9         | 65.70        | 77.70         | 23.70         | 43.70       | 5
